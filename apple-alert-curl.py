...
def search_hannaford_apples():
    """Search Hannaford.com for apple products using curl"""
    base_url = "https://www.hannaford.com/search/product"
    params = {
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"[debug] Using curl to fetch: {full_url}")
            
            # Use curl with the same headers that work
            curl_cmd = [
                'curl',
                '-s',  # silent
                '-L',  # follow redirects
                '--compressed',  # accept gzip/deflate
                '--max-time', '45',  # 45 second timeout
            ...
            if result.returncode == 0:
                print(f"[debug] Curl succeeded! Response length: {len(result.stdout)} chars")
                return result.stdout
            else:
                print(f"[warn] Curl failed with return code {result.returncode}")
                if result.stderr:
                    print(f"[warn] Curl stderr: {result.stderr.strip()}")
                if attempt == max_retries - 1:
                    print("[error] All curl attempts failed")
